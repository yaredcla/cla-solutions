    ...
    <main className="min-h-screen bg-white text-ink transition-colors dark:bg-slate-950 dark:text-white">
          ...
          <Reveal className="relative min-h-[500px]">
            ...
            <div className="absolute inset-x-0 bottom-0 mx-auto max-w-[620px] rounded-md border border-slate-200 bg-white p-4 shadow-glow dark:border-white/10 dark:bg-slate-950">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {["Traffic", "Leads", "Speed", "Trust"].map((label, index) => (
                  <div key={label} className="rounded-md bg-slate-50 p-4 dark:bg-slate-900">
                    <div className="mb-3 flex items-center justify-between">
                      <span className="text-sm font-semibold">{label}</span>
                      <span className="text-sm text-slate-500 dark:text-slate-400">+{24 + index * 11}%</span>
                    </div>
                    <div className="h-2 rounded-full bg-slate-200 dark:bg-white/10">
                      <div className="h-2 rounded-full bg-gradient-to-r from-brand to-cyan" style={{ width: `${68 + index * 7}%` }} />
                    </div>
                  </div>
                ))}
              </div>
              <div className="mt-4 h-40 rounded-md bg-ink p-4 dark:bg-black/30">
                <div className="flex h-full items-end gap-2">
                  {[44, 66, 48, 78, 56, 92, 72, 96, 84, 100].map((height, index) => (
                    <span
                      key={height}
      ...
      <section className="border-y border-slate-200 bg-white py-5 dark:border-white/10 dark:bg-slate-950">
        <div className="mx-auto grid max-w-7xl grid-cols-2 gap-3 px-4 sm:grid-cols-3 sm:px-6 lg:grid-cols-6 lg:px-8">
          ...
          <div key={item.label} className="flex items-center gap-2 rounded-md px-2 py-2 text-sm font-semibold text-slate-700 dark:text-slate-200">
              <item.icon className="text-brand dark:text-cyan" size={18} />
              {item.label}
            </div>
          ))}
        </div>
      ...
      <section id="services" className="px-4 py-20 sm:px-6 lg:px-8">
        <SectionHeading eyebrow="Services" title="Digital solutions for serious growth" copy="From elegant company sites to operational web apps, CLA Solutions builds the digital foundation your business needs to compete with confidence." />
      ...
      <section className="bg-slate-50 px-4 py-20 dark:bg-slate-950 sm:px-6 lg:px-8">
        <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[0.95fr_1.05fr]">
          <Reveal>
            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-brand dark:text-cyan">Why choose us</p>
            <h2 className="mt-3 text-3xl font-bold md:text-5xl">Built for African businesses with global standards.</h2>
            <p className="mt-5 text-base leading-7 text-slate-600 dark:text-slate-300">
              We combine local market understanding with modern engineering practices so your website can earn trust, convert customers, and scale with your ambition.
            </p>
            <div className="mt-8 grid gap-3 sm:grid-cols-2">
              {features.map((feature) => (
                <div key={feature} className="flex items-center gap-3 rounded-md bg-white p-3 text-sm font-semibold text-slate-800 shadow-sm dark:bg-slate-900 dark:text-slate-100">
                  <Check size={17} className="text-cyan" /> {feature}
                </div>
              ))}
            </div>
          </Reveal>
          <div className="grid gap-4 sm:grid-cols-2">
            {metrics.map((metric) => (
              <Reveal key={metric.label} className="rounded-md border border-slate-200 bg-white p-7 text-slate-800 shadow-sm dark:border-white/10 dark:bg-slate-950 dark:text-slate-100">
                ...
                <p className="mt-3 text-sm leading-6 text-slate-500 dark:text-slate-400">Every engagement is planned around reliability, clarity, and measurable business progress.</p>
              </Reveal>
            ))}
          </div>
        </div>
      ...
      <section id="work" className="px-4 py-20 sm:px-6 lg:px-8">
        <SectionHeading eyebrow="Portfolio" title="Realistic projects for regional growth" copy="Sample work concepts showing the kind of modern, conversion-focused platforms CLA Solutions can deliver." />
        <div className="mx-auto mt-12 grid max-w-7xl gap-5 md:grid-cols-2 xl:grid-cols-3">
      ...
      <section id="process" className="bg-ink px-4 py-20 text-white dark:bg-black sm:px-6 lg:px-8">
        <SectionHeading eyebrow="Process" title="A clear path from idea to launch" copy="You always know what is happening, what comes next, and how each step supports business outcomes." inverted />
        <div className="mx-auto mt-12 grid max-w-7xl gap-4 md:grid-cols-3 lg:grid-cols-6">
          {process.map(([title, copy], index) => (
            <Reveal key={title} className="rounded-md border border-white/10 bg-white/10 p-5">
      ...
      <section className="px-4 py-20 sm:px-6 lg:px-8">
        <SectionHeading eyebrow="Clients" title="Trusted by ambitious operators" copy="Realistic testimonials from business leaders who care about clarity, speed, and practical results." />
        <div className="mx-auto mt-12 grid max-w-7xl gap-5 lg:grid-cols-3">
          {testimonials.map((testimonial) => (
            <Reveal key={testimonial.name} className="rounded-md border border-slate-200 bg-white p-6 text-slate-800 shadow-sm dark:border-white/10 dark:bg-slate-900 dark:text-slate-100">
      ...
      <section className="bg-slate-50 px-4 py-20 dark:bg-slate-950 sm:px-6 lg:px-8">
        <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-2">
          <Reveal>
            ...
            <h2 className="mt-3 text-3xl font-bold md:text-5xl">Answers before the first call.</h2>
            <div className="mt-8 grid gap-4">
              {faqs.map((faq) => (
                <details key={faq.question} className="rounded-md border border-slate-200 bg-white p-5 text-slate-800 dark:border-white/10 dark:bg-slate-900 dark:text-slate-100">
          ...
          <div id="insights">
            <Reveal className="lg:pt-16">
              ...
              <h2 className="mt-3 text-3xl font-bold">Blog preview</h2>
              <div className="mt-8 grid gap-4">
                {blogPosts.map((post) => (
                  <article key={post.title} className="rounded-md border border-slate-200 bg-white p-5 text-slate-800 transition hover:border-brand dark:border-white/10 dark:bg-slate-900 dark:text-slate-100 dark:hover:border-cyan">
      ...
      <section className="px-4 py-20 sm:px-6 lg:px-8">
        <Reveal className="mx-auto max-w-6xl rounded-md bg-[linear-gradient(135deg,#0F172A,#1d4ed8_58%,#06B6D4)] px-6 py-14 text-center text-white shadow-glow md:px-12">
          <h2 className="text-3xl font-black md:text-5xl">Ready to Grow Your Business Online?</h2>
          <p className="mx-auto mt-4 max-w-2xl text-base leading-7 text-blue-50">Let&apos;s build a digital presence that helps your business stand out.</p>
      ...
      <section id="contact" className="bg-slate-50 px-4 py-20 dark:bg-slate-950 sm:px-6 lg:px-8">
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[0.9fr_1.1fr]">
          <Reveal>
            ...
            <h2 className="mt-3 text-3xl font-bold md:text-5xl">Tell us what you want to build.</h2>
            <p className="mt-5 text-base leading-7 text-slate-600 dark:text-slate-300">
        ...
        <div className="mx-auto mt-10 flex max-w-7xl flex-col gap-4 border-t border-white/10 pt-6 text-sm text-slate-300 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex gap-4">
            {["LinkedIn", "Facebook", "Telegram", "Instagram"].map((social) => (
              <a key={social} href="#top" className="hover:text-cyan">{social}</a>
            ))}
          </div>
        </div>
      </footer>
